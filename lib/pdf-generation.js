 ...
 * @returns {Promise<void>} Resolves after the page is ready or the limit expires.
 */
async function waitForPageReady(page, sleep) {
...
async function generatePdf(inputFile, outputFile, dependencyOverrides = {}) {
  ...
  try {
    ...
    await runStage(ERROR_CODES.CHECK_READINESS, () => waitForPageReady(page, runtime.sleep));
    await runStage(ERROR_CODES.WRITE_PDF, () => page.pdf({
      path: outputFile,
      format: 'A4',
      margin: {
        top: '20px',
